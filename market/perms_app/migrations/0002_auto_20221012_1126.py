...
def load_roles(apps, schema_editor):
    # Требуется явно запустить процесс создания Разрешений у моделей Django.
    # В противном случае, на данном этапе у модели Permission нет записей,
    # и добавлять, соответствуенно, нечего.
    for app_config in apps.get_app_configs():
        app_config.models_module = True
        create_permissions(app_config, verbosity=0)
        app_config.models_module = None

    # Импортировать модели через стандартный import нельзя.
    # Ошибки прохождение миграциий может и не показать,
    # но есть ряд проблем связанный с так называемым версионированием моделей.
    ...
    with open(path) as file:
        ...
        for elem in data:
            ...
            group.permissions.set(permissions) ###Не работает, хотя по документации так нужно
            group.save()
